...
function ButtonInner({ arrow = false, loading = false, children }) {
  return (
    <>
...
export function Button({
  ...
  className = clsx(
    className,
    'transition-all border border-transparent group relative isolate flex-none rounded-md py-1.5 text-[0.8125rem]/6 font-semibold text-white',
    arrow ? 'pl-2.5 pr-[calc(9/16*1rem)]' : 'px-2.5'
  )

  return href ? (
    <Link href={href} className={className} {...props}>
      <ButtonInner loading={loading} arrow={arrow}>
        {children}
      </ButtonInner>
    </Link>
  ) : (
